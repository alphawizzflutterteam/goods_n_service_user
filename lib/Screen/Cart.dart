...
import '../Helper/String.dart';
import '../Helper/Stripe_Service.dart';
import '../Model/Area_Charge_model.dart';
import '../Model/Model.dart';
import '../Model/Section_Model.dart';
import '../Model/User.dart';
import 'Add_Address.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(CUR_CURRENCY! +
                                          "${totalamount.toString()}" != " " ?
                                      CUR_CURRENCY! +
                                          "${double.parse(totalamount.toString())+ delCharge - promoAmt}":"",
                                        // CUR_CURRENCY! +
                                        //     " ${oriPrice.toStringAsFixed(2)}",
                                        style: TextStyle(
                                            color: Theme.of(context)
  ...
  String? areaCharge;
  AreaChargeModel ? areaChargeModel;
  getAdd() async {
    var headers = {
      'Cookie': 'ci_session=9dafe900508295d6afaa71fc46e37a4dd2090c8d'
    };
    var request = http.MultipartRequest('POST', Uri.parse('https://developmentalphawizz.com/goodsnservice/app/v1/api/get_address'));
    request.fields.addAll({
      'user_id': CUR_USERID.toString()
    });
    print("111111111111111111111${request.fields}");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
       var result = await response.stream.bytesToString();
       var finalResult = AreaChargeModel.fromJson(jsonDecode(result));
       for(int i =0; i<finalResult.data!.length; i++){
         if(finalResult.data![i].area==null||finalResult.data![i].area!.isEmpty){
           dCharge = 10;
         }else{
           areaCharge = finalResult.data?[i].deliveryCharges.toString();
         }


       }
     print("1111111111111111${areaCharge}");
  ...
  Future<void> placeOrder(String? tranId) async {
    double? finalAmount = double.parse(totalamount.toString())+ delCharge - promoAmt;
    print('---------FinalAmount--------${finalAmount}');
    print('---------OriPriceAmount--------${oriPrice.toString()}');
    print('---------TotalAmount--------${totalPrice.toString()}');
    print('---------Delcharge--------${delCharge}');
